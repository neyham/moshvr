...
public final class TerminalView extends View {
    ...
    private long mLastMouseEmitAt = -1;
    private int mLastMouseEmitButton = -1;
    private boolean mLastMouseEmitPressed;
    private float mStickAxis;
    private final Runnable mStickTick = this::tickStick;
    private long mLastWheelAt;
    private float mLastPointerX = -1f;
    private float mLastPointerY = -1f;
    public java.util.function.IntConsumer onExternalScroll;
    ...
    public TerminalView(Context context, AttributeSet attributes) { // NO_UCD (unused code)
        ...
        mGestureRecognizer = new GestureAndScaleRecognizer(context, new GestureAndScaleRecognizer.Listener() {
            ...
            public boolean onUp(MotionEvent event) {
                ...
                if (mEmulator != null && shouldReportMouseClick(event) && !event.isFromSource(InputDevice.SOURCE_MOUSE) && !isSelectingText() && !scrolledWithFinger) {
    ...
    /**
     * Horizon 2D panels only forward the thumbstick as a scroll to the view
     * under the ray when this returns true. herdr's alt buffer has no
     * transcript, so the default implementation was false and the stick died.
     */
    @Override
    ...
    protected int computeVerticalScrollRange() {
        ...
        if (history <= 0) return mEmulator.mRows * 4;
    ...
    /**
     * Forward a click only when the remote enabled DEC 1000/1002/1003.
     * Alt-screen alone is not enough: herdr is always on the alt screen, and
     * X10/SGR bytes it does not consume print as garbage.
     */
    boolean shouldReportMouseClick(MotionEvent event) {
    ...
    void sendMouseEventCode(MotionEvent e, int button, boolean pressed) {
        ...
        // Quest delivers the same click as BUTTON_PRESS and ACTION_DOWN.
        // Do not debounce wheel ticks — herdr needs every notch.
        boolean wheel = button == TerminalEmulator.MOUSE_WHEELUP_BUTTON ||
            button == TerminalEmulator.MOUSE_WHEELDOWN_BUTTON;
        long now = SystemClock.uptimeMillis();
        if (!wheel && button == mLastMouseEmitButton && pressed == mLastMouseEmitPressed && now - mLastMouseEmitAt < 40) {
            return;
        }
        mLastMouseEmitAt = now;
        mLastMouseEmitButton = button;
        mLastMouseEmitPressed = pressed;
    ...
    /** Scroll the terminal, or send wheel/arrows when a TUI is reading those. */
    private boolean acceptWheelTick() {
        long now = SystemClock.uptimeMillis();
        if (!StickScroll.acceptTick(mLastWheelAt, now)) return false;
        mLastWheelAt = now;
        return true;
    }

    public void scrollLines(int rowsDown) {
        if (mEmulator == null || rowsDown == 0) return;
        if (!acceptWheelTick()) return;
        // Never use last pointer: a click on herdr/chrome tabs would wheel those tabs.
        float x = getWidth() / 2f;
        float y = getHeight() * 0.55f;
        MotionEvent ev = MotionEvent.obtain(SystemClock.uptimeMillis(), SystemClock.uptimeMillis(), MotionEvent.ACTION_MOVE, x, y, 0);
        doScroll(ev, rowsDown);
        ev.recycle();
        if (onExternalScroll != null) onExternalScroll.accept(rowsDown);
        android.util.Log.i("MoshVrStick", "scrollLines " + rowsDown);
    }

    private boolean applyStickAxis(float axis) {
        mStickAxis = axis;
        removeCallbacks(mStickTick);
        if (StickScroll.rowsForAxis(axis) == 0) return true;
        tickStick();
        return true;
    }

    private void tickStick() {
        int rows = StickScroll.rowsForAxis(mStickAxis);
        if (rows == 0 || mEmulator == null) return;
        scrollLines(rows);
        postDelayed(mStickTick, StickScroll.delayMs(mStickAxis));
    }
    ...
    public boolean onGenericMotionEvent(MotionEvent event) {
        ...
        if (event.isFromSource(InputDevice.SOURCE_MOUSE)) {
            ...
            if (action == MotionEvent.ACTION_SCROLL) {
                if (!acceptWheelTick()) return true;
                boolean up = event.getAxisValue(MotionEvent.AXIS_VSCROLL) > 0.0f;
                int rows = up ? -1 : 1;
                doScroll(event, rows);
                if (onExternalScroll != null) onExternalScroll.accept(rows);
                return true;
            }
            if (shouldReportMouseClick(event) &&
                (action == MotionEvent.ACTION_BUTTON_PRESS || action == MotionEvent.ACTION_BUTTON_RELEASE)) {
                sendMouseEventCode(event, TerminalEmulator.MOUSE_LEFT_BUTTON, action == MotionEvent.ACTION_BUTTON_PRESS);
                return true;
            }
            // Pointing controller: AXIS_Y is a pixel, not a stick. Use hat / right-stick axes.
            float pointedStick = StickScroll.strongestVertical(
                0f,
                event.getAxisValue(MotionEvent.AXIS_RZ),
                event.getAxisValue(MotionEvent.AXIS_RY),
                event.getAxisValue(MotionEvent.AXIS_HAT_Y)
            );
            if (StickScroll.rowsForAxis(pointedStick) != 0 || StickScroll.rowsForAxis(mStickAxis) != 0) {
                return applyStickAxis(pointedStick);
            }
        }
        if (event.isFromSource(InputDevice.SOURCE_JOYSTICK) || event.isFromSource(InputDevice.SOURCE_GAMEPAD)) {
            float axis = StickScroll.strongestVertical(
                event.getAxisValue(MotionEvent.AXIS_Y),
                event.getAxisValue(MotionEvent.AXIS_RZ),
                event.getAxisValue(MotionEvent.AXIS_RY),
                event.getAxisValue(MotionEvent.AXIS_HAT_Y)
            );
            return applyStickAxis(axis);
        }
        return false;
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (isSelectingText()) {
            ...
            } else if (shouldReportMouseClick(event)) { // BUTTON_PRIMARY. Quest controller is SOURCE_MOUSE.
    ...
    public void updateSize() {
        ...
        int newRows = Math.max(4, (viewHeight - mRenderer.mFontLineSpacingAndAscent - mRenderer.getFontDescent()) / mRenderer.mFontLineSpacing);
